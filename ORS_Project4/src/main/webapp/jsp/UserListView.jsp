...
<body>
	...
	<form action="<%=ORSView.USER_LIST_CTL%>" method="post">
		...
		<center>
			...
			//List Dlist = (List) request.getAttribute("RoleList");
			...
			<table width="100%" align="center">
				<tr>
					...
					<td align="center"><label>FirstName</font> :
						...
						&emsp; 
						<%-- <label>DOB</font>:
 <%-- 						</label><input type="text" name="DOB" placeholder="Enter DOB" value="<%=ServletUtility.getParameter("DOB", request)%>"> --%>
 				<label>Role</font> :</label> <%=HTMLUtility.getList("roleid", String.valueOf(bean.getRoleId()), rlist)%>
 				
 				<%-- <label>DOB</font> :</label><%=HTMLUtility.getList("DOB", String.valueOf(bean.getDob()), Dlist) %>
			 --%>			&nbsp; <%-- <%=HTMLUtility.getList("loginid", String.valueOf(bean.getRoleId()), ulist)%>
 --%> &nbsp;
                   <%--  <label>LastName<font>:<input type="text" name="LastName" placeholder="Enter Last Name" 
                                   value="<%=ServletUtility.getParameter("LastName", request)  %>"></label></font> 
  --%>
