...
public class PatientModel {
	...
	public long add(PatientBean bean) throws Exception {
		...
		int pk = 0;
		pk = nextPK();
		PreparedStatement ps = conn.prepareStatement("INSERT INTO ST_PATIENT VALUES(?,?,?,?,?)");

		ps.setLong(1, pk);
		ps.setString(2, bean.getName());
		ps.setDate(3, new Date(bean.getDob().getTime()));
		ps.setLong(4, bean.getMobileNo());
		ps.setString(5, bean.getDecease());
	...
	public void update(PatientBean bean) throws Exception {
		...
		PreparedStatement ps = conn
				.prepareStatement("UPDATE ST_PATIENT SET NAME=?, DOB=?, MOBILE_NO=?, DECEASE=? WHERE ID=?");

		ps.setString(1, bean.getName());
		ps.setDate(2, new Date(bean.getDob().getTime()));
		ps.setLong(3, bean.getMobileNo());

		ps.setString(4, bean.getDecease());
	...
	public void delete(PatientBean bean) throws Exception {
		...
		Connection conn = JDBCDataSource.getConnection();
		PreparedStatement ps = conn.prepareStatement("DELETE FROM ST_PATIENT WHERE ID=?");
